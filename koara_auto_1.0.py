...
def main():
	...
	with open('urllist.txt') as f:
		for line in f:
			links.append(line[:-1])
		f.close()
	for link in links: #put the number of the url you want to start from here		
		filename = getinfo(link)
		getjpgs(link, filename, automan)
		if link == links[-1]:
			print('All finished!')
		else:
			print('Waiting 30 seconds until next download...')
			time.sleep(30)
...
def scraper(page_url, num_images):
	...
	for jpg in range(1, num_images + 1):
		...
		#print('Downloading {0}{1}.jpg'.format(page_url, jpg))
